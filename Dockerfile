...
COPY src/jose/jose-${VERSION}.tar.xz LICENSE src/jose/jose.info src/jose/jose.SlackBuild src/jose/README src/jose/slack-desc /root/build-jose/
WORKDIR /root/build-jose/
...
# Build the package
RUN echo RUN VERSION="$VERSION" TAG="$TAG" BUILD="$BUILD" TMP="/tmp/$TAG" ./jose.SlackBuild
RUN VERSION="$VERSION" TAG="$TAG" BUILD="$BUILD" TMP="/tmp/$TAG" ./jose.SlackBuild
